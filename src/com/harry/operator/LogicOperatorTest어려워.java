...
public class LogicOperatorTest어려워 {
	public static void main(String[] args) {
		boolean isOnTv = true;
		boolean isOnRadio = true;

		if (isOnTv && isOnRadio) {// 앞의 피연산자를 검사하여 false면 뒤의 연산자를
									// 검사하지 않습니다. 효율적이므로 실무에서 많이 쓰임.
		...
		if (isOnTv & isOnRadio) {// 피연산자2개를 모두 검사합니다(비효율적,실무에서 안씀.)
			System.out.println("AND(&) is true..isOnTv isOnRadio");
			System.out.println("isOnTv value is " + isOnTv);
			System.out.println("isONRadio value is " + isOnRadio);
		} else {
